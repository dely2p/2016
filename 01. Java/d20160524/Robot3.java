...
public class Robot3 extends Frame implements KeyListener,MouseMotionListener{
	Button[] btn;
	Button u;
	int key;
	...
	public void keyPressed(KeyEvent e) { // 키보드 누를 때
		...
		for(int j=0;j<btn.length;j++){
			...
			//+= sin(Angle * 3.14 / 180) + Power - ( 0.98 * Time * Time) / 2
				System.out.println("x : "+btn[0].getX()+" , y : "+btn[0].getY());
				for(int i=0;i<50;i++){
					for(int j=0;j<btn.length;j++){
						x = btn[j].getX();
						y = btn[j].getY();
					...
					/*	x += (int)Math.cos(45);
						y += (int)(Math.sin(45)-1/2*9.8);*/
						//int py = (int)(-0.05*x*x-0.5*x-20);
